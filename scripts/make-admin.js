 ...
 *
 * Для снятия админки:
 *   node scripts/make-admin.js email@example.com --remove
 */
...
async function makeAdmin(email, remove = false) {
  try {
    ...
    console.log(`   Роль: ${resultUser.role === 'admin' ? '👑 Администратор' : '👤 Пользователь'}`)
...
if (!email) {
  ...
  console.log('   node scripts/make-admin.js email@example.com --remove')
